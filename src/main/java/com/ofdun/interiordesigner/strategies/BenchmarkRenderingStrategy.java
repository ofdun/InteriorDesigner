...
public class BenchmarkRenderingStrategy implements RenderingStrategy {

    private static final double EPSILON = 1e-6;
    private final AtomicInteger pixelsRendered = new AtomicInteger(0);
    private long lastRenderTimeNanos = 0;

    @Override
    public void renderPixel(int x, int y, PixelRenderContext context) {
        renderPixelInternal(x, y, context);
    }
    ...
    private void renderPixelInternal(int x, int y, PixelRenderContext context) {
        ...
        if (coords.isInsideTriangle()) {
            ...
            if (context.zBuffer().testAndSet(x, y, z)) {
                ...
                calculateColor(interpolatedWorldPos, interpolatedNormal, coords, z1, z2, z3, context);
    ...
    public void printStatistics() {
        System.out.println("Обработано пикселей: " + pixelsRendered.get());
    }

    @Override
    public void resetStatistics() {
        pixelsRendered.set(0);
        lastRenderTimeNanos = 0;
    }

    public int getPixelsRendered() {
        return pixelsRendered.get();
    }

    public void startRenderTimer() {
        lastRenderTimeNanos = System.nanoTime();
    }

    public long stopRenderTimer() {
        if (lastRenderTimeNanos == 0) {
            return 0;
        }
        long elapsed = (System.nanoTime() - lastRenderTimeNanos) / 1_000_000;
        lastRenderTimeNanos = 0;
        return elapsed;
    }
